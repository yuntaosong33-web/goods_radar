...
```text
collect sources -> scan leads -> collect route stats -> rule baseline -> Codex evaluation -> Node guardrails -> reports/tasks/weekly review
```
...
2. Node builds Codex cases with company, route, evidence, baseline, and guardrails.
...
## Route Stats And Optional Bill Validation
...
The verifier checks data headers, canonical levels/statuses, route rows, bill rows, LLM audit rows, report links, and pipeline formatting.
