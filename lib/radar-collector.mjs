...
function latestProviderCapabilityPath() {
  ...
  for (const date of dates) {
    const candidate = join(root, date, 'factory-capabilities.tsv');
    if (existsSync(candidate)) return candidate;
  }
  return '';
}

function providerHealthRows(providerRows, collectedAt) {
  ...
  for (const row of providerRows) {
    ...
    const current = counts.get(id) || {
      ...
      layer: 'source_provider',
      status: 'usable',
      retrieval: 'provider staging capability rows',
      row_count: 0,
    ...
    current.row_count += 1;
    if (!current.source_url && row.source_url) current.source_url = clean(row.source_url);
...
export async function collectRadarSources({
  ...
  providerCapabilityPath = latestProviderCapabilityPath(),
  collectedAt = todayIso(),
} = {}) {
  const rows = fixture ? fixtureRadarRows({ collectedAt }) : (() => {
    const { rows: autoLeads } = readTsv(autoLeadPath);
    const { rows: providerCapabilities } = providerCapabilityPath
      ? readTsv(providerCapabilityPath, FACTORY_CAPABILITY_HEADERS)
      : { rows: [] };
    return {
      capabilities: [
        ...autoLeads.map(row => capabilityFromAutoLead(row, collectedAt)).filter(Boolean),
        ...providerCapabilities.filter(row => row.source_url || row.url_or_file),
      ],
      capacities: [],
      approvals: [],
    };
  ...
  const baseHealth = RADAR_SOURCE_DEFINITIONS.map(source => {
  ...
  const providerRows = rows.capabilities.filter(row => !RADAR_SOURCE_DEFINITIONS.some(source => source.id === row.source_id));
  const health = [...baseHealth, ...providerHealthRows(providerRows, collectedAt)];
...
export function radarSourceHealthSummary(health) {
  const statusLabel = {
    ...
    no_structured_rows: '未解析到结构化行',
    error: '异常',
  };
  const reasonLabel = value => String(value || '')
    .replace(/Manual official export or parser required/g, '需要人工导出官方文件或维护解析器')
    .replace(/No matched official capability rows/g, '未匹配到官方能力行')
    .replace(/OK/g, '正常');
  return (health || [])
    .map(row => `${row.source_id} | ${statusLabel[row.status] || row.status} | ${row.row_count} | ${reasonLabel(row.reason)}`)
    .join('\n');
}
