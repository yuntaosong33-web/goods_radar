...
function taskFor(row) {
  ...
  return {
    ...
    country: row.country,
    city: row.city,
    task_name: `核实 ${row.normalized_company_name} 是否存在可开发牛百叶货源`,
    task_type: '电话/拍照/询价',
    assignee: '',
    must_ask: '是否有牛百叶/omaso/librillo/folhoso;每周屠宰或收集量;当前卖给谁;是否与 rumen/honeycomb 混装;是否能清洗盐腌分拣包装;是否允许现场看货;是否能发海防/香港/中国',
...
export function formatRuleScoreSummary({ changed, newTasks, scoredRows, radarRows = [] }) {
  ...
  if (top.length) {
    ...
    for (const row of top) {
      lines.push(`- ${row.normalized_company_name} | ${countryLabel(row.country)} | ${row.omasum_level}/${row.evidence_level}/${row.development_distance} | ${row.score}/100 | ${row.status}`);
    }
  }
  return lines.join('\n');
}
